...
def benchmark_mult(dim1: int, dim2: int, dim3: int, repetitions: int) -> None:
    ...
    best_possible_time = max(2.27e-11 * (dim1 * dim2 + dim2 * dim3 + dim1 * dim3),
                             4.62e-13 * (dim1 * dim2 * dim3))  # Based on my hardware (NVIDIA GeForce RTX 3050 Laptop GPU)
    ...
    best_possible_bandwidth = bytes_moved / best_possible_time
    pytorch_bandwidth = bytes_moved / pytorch_mean
    custom_bandwidth = bytes_moved / custom_mean

    string = f"""
Shape: {dim1} x {dim2} @ {dim2} x {dim3}
    Execution Times:
        Best possible: {best_possible_time*1e3:.3f} ms
        PyTorch: {pytorch_mean*1e3:.3f} ms
        Custom: {custom_mean*1e3:.3f} ms
    Bandwidths:
        Best possible: {best_possible_bandwidth/1e9:.3f} GB/s
        PyTorch: {pytorch_bandwidth/1e9:.3f} GB/s
        Custom: {custom_bandwidth/1e9:.3f} GB/s
    """
    ...
    if custom_mean < pytorch_mean:
        print(f"{GREEN}Custom multiplication is {(pytorch_mean - custom_mean) / pytorch_mean * 100:.3f}% faster than PyTorch multiplication{RESET}")
    else:
        print(f"{RED}Custom multiplication is {(custom_mean - pytorch_mean) / pytorch_mean * 100:.3f}% slower than PyTorch multiplication{RESET}")
    print(f"{RED}Custom multiplication is {(custom_mean - best_possible_time) / best_possible_time * 100:.3f}% slower than the best possible time{RESET}")
    print("-" * 80)
